...
def load_data(file_path, sample_fraction=0.05, chunk_size=10000) -> pd.DataFrame:
...
def clean_data_types(data: pd.DataFrame) -> pd.DataFrame:
    ...
    indicator_cols = [
        'Single_borrower', 'Number_of_units', 'DFlag'
    ]
    for col in indicator_cols:
        data[col] = data[col].astype(str)

    column_prefixes = [
        'is_Loan_purpose', 'is_First_time_', 'is_Occupancy_status_',
...
def preprocess_data(df):
    ...
    # origicopy to preserve original categorical values
    original_df = df.copy()
    ...
    return X_scaled, y, scaler, original_df
...
def shap_analysis(model, X, feature_names):
    ...
    # Handle multi-class models (eg XBoost with probabilities)
    try:
        shap_values = explainer.shap_values(X)
        if isinstance(shap_values, list): # -- multi-class models return list
            shap_values = shap_values[1] #-- Use class index 1
...
def main():
    ...
    file_path = st.sidebar.text_input("Enter Data Path", os.path.join(data_folder, "red_train_12.csv"))
